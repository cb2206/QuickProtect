# App Store pipeline (Fastlane)

> Note: `fastlane/README.md` is auto-generated by Fastlane on every run (and
> committed so the lane list is browsable) — keep human docs here in
> `PIPELINE.md` instead.

Automates App Store Connect for QuickProtect: localized metadata, build upload,
and submission. macOS app, bundle id `com.cb.quickprotect`, team `K2LM9FSU6C`.

## One-time setup

1. **Fastlane** via Homebrew (`/opt/homebrew/bin/fastlane`). Reinstall: `brew install fastlane`.
2. **App Store Connect API key** — *Users and Access → Integrations → App Store
   Connect API → Generate API Key*. **Role must be `Admin`** — cloud signing
   needs certificate management, which App Manager cannot do. Download the
   `AuthKey_XXXX.p8` once; store it in `~/.appstoreconnect/`.
3. Copy `fastlane/.env.example` → `fastlane/.env` with `ASC_KEY_ID`,
   `ASC_ISSUER_ID`, `ASC_KEY_PATH`. `.env` and `*.p8` are gitignored.

## Source of truth

`docs/APP_STORE_LISTINGS.md` (repo root) holds every localized listing. Editing it
and running `tools/gen_appstore_metadata.py` (from `macos/`) regenerates
`fastlane/metadata/<locale>/` (with char-limit checks). Don't hand-edit the
generated `.txt` files.

Locale map (app token → ASC): en→en-US, de→de-DE, fr→fr-FR, es→**es-ES**,
nl→nl-NL, it→it, pt-BR→pt-BR. (Change es in the generator for es-MX.)

## Lanes (run from repo root, `LANG=en_US.UTF-8` silences a UTF-8 warning)

| Lane | What it does |
|---|---|
| `auth_check` | Read-only: verify API auth + print version states |
| `status` | Read-only: version states + recent build processing |
| `metadata` | Regenerate + push all 7 listings (no binary) — safe anytime |
| `build` | Archive Release + export a signed App Store `.pkg` (cloud signing) |
| `upload_build` | Upload an exported `.pkg` to App Store Connect (no submit) |
| `cancel_review` | Remove the current version from review (frees the slot) |
| `resubmit` | Upload an already-exported `.pkg` + resubmit the current version (no rebuild, no metadata) |
| `submit_only` | Submit the current version with an already-uploaded build (`build:6` option or `SUBMIT_BUILD` env) — for when the binary is up but the submit failed |
| `release` | build → upload → metadata → submit (the going-forward one-shot) |
| `review_subs` | Read-only: list review submissions (state + platform + items) |
| `review_detail` | Read-only: dump the App Review contact + notes for every version |
| `ci_status` / `ci_disable` | Read-only list / disable all Xcode Cloud workflows |
| `delete_stray_ios` | Cleanup: delete stray non-macOS editable version drafts |
| `gen` | Regenerate the Xcode project from project.yml |

App Review contact details and per-submission review notes are set in App
Store Connect, not in the Fastfile — the repository is public, and the
one-off `submit_v11`/`submit_v12` lanes that used to carry them were removed.

## Gotchas (learned the hard way)

- **`platform: "osx"`** is required on every `deliver` call — it defaults to
  `ios` and will otherwise create a phantom iOS version / fail to find the build.
- The **Admin** key requirement above (cloud signing).
- After `cancel_review`, the version goes to `DEVELOPER_REJECTED` (editable),
  not `PREPARE_FOR_SUBMISSION` — that's normal for a self-withdrawn version.
- Screenshots aren't automated (the `screenshots/` dir isn't in deliver's
  `metadata/<locale>/` layout). Manage those in App Store Connect.
- **Building on a beta macOS ⇒ ITMS-90301** ("not currently accepting
  applications built with this version of the OS"), even with a release Xcode
  and SDK — the validator rejects the beta `BuildMachineOSBuild` stamp (e.g.
  `26A5388g`) in the app's Info.plist. Happened with 1.3 build 5 (2026-08-10).
  Workaround from a beta machine: bump `CFBundleVersion` in `project.yml`,
  archive with `xcodebuild archive` (cloud-signing auth args), patch the stamp
  in the **archive** to the current public build
  (`PlistBuddy -c 'Set :BuildMachineOSBuild <public>' …/Products/Applications/QuickProtect.app/Contents/Info.plist`),
  then `xcodebuild -exportArchive` (signing happens at export, so the patched
  plist is what gets signed), copy the pkg over `build/fastlane/QuickProtect.pkg`,
  and run `fastlane mac resubmit`. Long-term fix: build on a release-OS machine.
- **"A review submission is already in progress"** when resubmitting after a
  binary rejection: the old review submission lingers in `UNRESOLVED_ISSUES`.
  Run `cancel_review` (the version stays editable with its metadata), then
  `submit_only` with the new build number — don't re-run `resubmit`, its pkg
  re-upload would trip the duplicate-binary check.
