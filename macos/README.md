...
**On Windows or Linux?** QuickProtect ships there too — Windows via the Microsoft Store or the unsigned installer on [GitHub Releases](https://github.com/cb2206/QuickProtect/releases), Linux as a tarball on the same releases. See the [.NET/Avalonia port](../dotnet/README.md).
...
To add or edit a translation, update the relevant dictionary in that script and re-run it (XcodeGen picks up new languages into `knownRegions` automatically; also add the language code to `CFBundleLocalizations` in `project.yml`). Localized App Store metadata for every language is kept in [`APP_STORE_LISTINGS.md`](../docs/APP_STORE_LISTINGS.md).
